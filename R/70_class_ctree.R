...
#apply the function
#splits <- splitdf(dataList, seed=808)

#index <- 1:nrow(dataList)
trainindexes <- 1:trunc(nrow(dataList)/3)
training <- dataList[trainindexes, ]
testing <- dataList[-trainindexes, ]
#splits <- list(trainset=training, testset=testing)
